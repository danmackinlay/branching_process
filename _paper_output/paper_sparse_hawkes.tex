...
The degrees of freedom for parametric models is taken to be the number
...
\(\pi\) in an \(\ell_1\) regression model provides an unbiassed estimate
...
\[
\hat{d}\left(N,\hat{\theta},\pi\right)=\sum_j\mathbb 1\left\{\hat{\theta_j}(N,p,q)\neq 0\right\}
\]
...
test the efficacy of the proposed method.
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{paper_sparse_hawkes_files/paper_sparse_hawkes_22_0.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{paper_sparse_hawkes_files/paper_sparse_hawkes_22_1.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{paper_sparse_hawkes_files/paper_sparse_hawkes_22_2.pdf}
    \end{center}
    { \hspace*{\fill} \\}
...
\href{https://github.com/scikit-optimize/scikit-optimize}{scikit-optimize}.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
        ...
        <ipython-input-8-765ef5deb338> in <module>()
          1 from skopt import load
          2 
    ----> 3 res\_gp = load(join(FIT\_DIR, 'additive\_cvge.gp.bz2'))
    

        /usr/local/lib/python3.6/site-packages/skopt/utils.py in load(filename, **kwargs)
        122         Reconstructed OptimizeResult instance.
        123     """
    --> 124     return load\_(filename, **kwargs)
    

        /usr/local/lib/python3.6/site-packages/sklearn/externals/joblib/numpy\_pickle.py in load(filename, mmap\_mode)
        565             obj = \_unpickle(fobj)
        566     else:
    --> 567         with open(filename, 'rb') as f:
        568             with \_read\_fileobject(f, filename, mmap\_mode) as fobj:
        569                 if isinstance(fobj, \_basestring):


        FileNotFoundError: [Errno 2] No such file or directory: '/Users/dan/Datasets/branching\_process\_output/additive\_cvge.gp.bz2'
